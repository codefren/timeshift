-- ============================================================================
-- Migración: corregir el trigger trg_UpdateUserHoursBalance
-- ----------------------------------------------------------------------------
-- La base restaurada trae una versión ANTIGUA del trigger que:
--   * solo cubre AFTER INSERT (no UPDATE/DELETE),
--   * referencia la columna inexistente WorkLogTotals.TotalPauseHours, y
--   * escribe en columnas viejas (PausedHours) que ya no existen en
--     UserWeekHoursBalance / UserTotalHoursBalance.
-- Resultado: CUALQUIER INSERT en WorkLogTotals falla (Msg 207) → no se puede
-- cerrar un worklog (fichar salida).
--
